...
class ClickUpWebhookController extends Controller
{
    ...
    public function handle(Request $request)
    {
        try {
                ...
                if (!$this->clickUp->verifySignature($raw, $sig)) {
                    ...
                    if (env('CLICKUP_ALLOW_UNVERIFIED', app()->environment('local'))) {
                        ...
                        ClickUpWebhookLog::create([
                            ...
                            'task_id' => null,
                            'status_code' => 401,
                            'message' => 'Invalid signature - rejected',
                            'payload' => json_decode($raw, true),
                        ]);
            ...
            // Check if API call failed (returned error)
            if (isset($task['__error'])) {
                ClickUpWebhookLog::create([
                    'event' => $event,
                    'task_id' => (string) $taskId,
                    'status_code' => $task['__error']['status'] ?? 500,
                    'message' => 'ClickUp API error: ' . ($task['__error']['body'] ?? 'Unknown error'),
                    'payload' => $payload,
                ]);
                // Still return 200 to ClickUp so it doesn't mark webhook as failed
                // The error is logged for investigation
                return response()->json(['message' => 'API error logged'], 200);
            }
            ...
            // Log all exceptions to webhook logs for debugging
            $payload = $request->all();
            ...
            Log::error('ClickUp webhook handler exception', [
            ...
            // Return 200 to prevent ClickUp from marking webhook as failed repeatedly
            // The error is logged for investigation
            return response()->json(['error' => 'Internal error logged'], 200);
        }
    }
}
